...
All buffers default to `RGBA` format (which is `RGBA8` on most systems), but this can be configured as described in the [Image Format](/current/reference/buffers/image_format) section.
...
*Note: if `gdepth` is defined anywhere in the shader and the texture/image format for `colortex1`/`gdepth` is `RGBA` (or not specifically set by the shader), Iris will upgrade the format to `RGBA32F`. This represents legacy behavior, where `gdepth` was used to store depth values.*